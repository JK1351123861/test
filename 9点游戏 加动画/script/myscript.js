...
Player.prototype.compare=function(player){
    if(this.getRank()>player.getRank()||
        this.getRank()>players.getRank())return 'lose';
    else if(this.getRank()<player.getRank()||
        this.getRank()<players.getRank())return 'win';
    else return 'win';
};
// 比较大小
...
window.onload=function(){
    names.forEach(function(name,index){
        suits.forEach(function(suit,i){
            if(index===0){
                cards.push(new Card(name,suit,1));
            }else if(index<9){
...
function start(){
    ...
    playersDiv.classList.remove('win');
    playersDiv.classList.remove('lose');
...
function downe(){
    document.getElementById('button_alone').style.backgroundImage="url(images/cards/button_alones.png)"
};
function upe(){
    document.getElementById('button_alone').style.backgroundImage="url(images/cards/button_alone.png)"
};
function downes(){
    document.getElementById('button_both').style.backgroundImage="url(images/cards/button_boths.png)"
}
function upes(){
    document.getElementById('button_both').style.backgroundImage="url(images/cards/button_both.png)"
};
// 开始抓牌
function getCard(){
    Zj.getCard();
...
function showCard(){
    ...
    var cardsList=document.querySelectorAll('#players .card');
    var hands=player.hands;
    var handss=players.hands;
    for(var i=0;i<cardsList.length;i++){
        cardList[i].style.backgroundImage='url(images/cards/'+hands[i].suit+'/'+hands[i].name+'.png)';
        cardsList[i].style.backgroundImage='url(images/cards/'+handss[i].suit+'/'+handss[i].name+'.png)';
    }
    var playerDiv=document.getElementById('player');
    var playersDiv=document.getElementById('players');
    var ZjDiv=document.getElementById('Zj');
    ...
    var result=Zj.compare(players);
    if(result==='lose'){
        ZjDiv.classList.add('lose');
        playersDiv.classList.add('win')
    }else if(result==='win'){
        ZjDiv.classList.add('win');
        playersDiv.classList.add('lose')
    };
    var result=player.compare(players);
    if(result==='lose'){
        playerDiv.classList.add('lose');
        playersDiv.classList.add('win')
    }else if(result==='win'){
